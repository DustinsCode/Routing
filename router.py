			...
			if ethType == '\x08\x06':
				...
				if ipContents[1] == '\x00' and ipProtocol == '\x01':
					...
					if icmpType == '\x08':
						...
						nextiface = myRouter.findNextHop(self, self.listIP1, socket.inet_ntoa(destinationIP))
						if nextiface == False:
							nextiface = myRouter.findNextHop(self, self.listIP2, socket.inet_ntoa(destinationIP))
